...
def _choose_anchors(extraction: ProjectExtraction, projeto: dict, page_no: int) -> list[Position]:
    ...
    for equipment_item in equipment:
        ...
        if distance <= 430 and (distance <= 330 or is_work_item):
...
def _source_region(extraction: ProjectExtraction, page_no: int, anchors: list[Position]) -> SourceRect:
    ...
    min_width = min(760.0, width)
    min_height = min(650.0, height)
    cx = (min_x + max_x) / 2
    cy = (min_y + max_y) / 2
    half_w = max((max_x - min_x) / 2 + 28, min_width / 2)
    half_h = max((max_y - min_y) / 2 + 28, min_height / 2)
...
def _header(c: canvas.Canvas, metadata: dict[str, str]) -> None:
    ...
    fields = [
        ("Departamento:", metadata.get("departamento", "SERRA")),
...
def _footer(c: canvas.Canvas) -> None:
    ...
    for index, question in enumerate(questions):
        ...
        c.drawString(x + w - 75, ry + 1.4, "Nao" if index == len(questions) - 1 else "Sim")
...
def render_croqui_geometrico(extraction: ProjectExtraction, projeto: dict, out_path: Path) -> Path:
    ...
    page_no = max(
        ...
        key=lambda page: sum(s.length for s in extraction.conductors if s.page == page),
    )
    anchors = _choose_anchors(extraction, projeto, page_no)
    region = _source_region(extraction, page_no, anchors)
    target = (28.0, 98.0, 565.0, 405.0)
    ...
    for pole in extraction.poles:
        ...
        if region.x0 <= pole.position.x <= region.x1 and region.y0 <= y_top <= region.y1:
            ...
            _draw_pole(c, px, py)
            selected_poles.append((pole, px, py))

    semantic_codes = set(_semantic_codes(projeto))
    rendered_codes: set[str] = set()
    work_positions: list[tuple[float, float]] = []
    for equipment in _all_equipment(extraction):
        ...
        if is_new or equipment.numero in semantic_codes:
            work_positions.append((x, y))

    if not work_positions and anchors:
        work_positions = [point(anchors[0].x, _page_y(anchors[0], extraction))]
    if work_positions:
        wx, wy = work_positions[-1]
        c.setStrokeColor(red)
        c.setLineWidth(1.5)
        c.setDash([2, 2])
        c.rect(wx - 23, wy - 34, 46, 68, fill=0, stroke=1)
        c.setStrokeColor(blue)
        c.ellipse(wx - 12, wy - 26, wx + 12, wy + 26, fill=0, stroke=1)
        c.setDash([])

    # Legenda de areas separada da geometria, como no modelo oficial.
    c.setStrokeColor(red)
    c.setLineWidth(1.4)
    c.setDash([2, 2])
    c.rect(625, 290, 75, 92, fill=0, stroke=1)
    c.setDash([])
    c.setFillColor(red)
    c.setFont("Helvetica-Bold", 5.5)
    c.drawString(715, 335, "AREA DE TRABALHO 1 LM")
    c.setStrokeColor(blue)
    c.setLineWidth(1.4)
    c.setDash([2, 2])
    c.ellipse(615, 220, 710, 260, fill=0, stroke=1)
    c.setDash([])
    c.setFillColor(blue)
    c.drawString(715, 237, "AREA DE TRABALHO 2 LV")
